...
namespace MagicPigGames.Northstar
{
    ...
    public class OverlayIcon : MonoBehaviour
    {
        ...
        public bool useDistanceToFade = true;
        ...
        */
        public float GetAngleDelta()
        {
            ...
            else
            {
                try
                {
                    result = _trackedTargetOverlay.fixedNavigationBarAngle
                ? NorthstarUtilities.GetAngleBetweenPointsSourceForwardIgnoringRotation(CameraTransform, _trackedTargetOverlay.fixedAngle, GlobalNorthstarSettings.Instance.northAngle)
                : NorthstarUtilities.GetAngleBetweenPointsSourceForwardIgnoringRotation(CameraTransform, TrackedPosition);
                }
        ...
        public virtual Camera TargetCamera => _northstarOverlay.targetCamera == null ? Camera.main : _northstarOverlay.targetCamera;
        public virtual Transform CameraTransform => TargetCamera == null ? null : TargetCamera.transform;
        ...
        protected virtual void SetNavigationBarValues()
        {
            ...
            FadeAtEdgesOfNavigationBar(GetAngleDelta());
        }
        ...
        protected virtual void SetValues()
        {
            ...
            float opacity = useDistanceToFade ? DistanceToTarget / overlayDistanceMax : 0;

            opacity = 1 - opacity;
            float value = FadeValueLookingToMarker();
            opacity = Mathf.Max(opacity, value);
            opacity = NorthstarOverlay.characterPlayer.IsAiming() ? 0 : opacity;

            //opacity = FadeValueLookingToMarker() ? 1 : opacity;

            _image.color = new Color(mainSpriteColor.r, mainSpriteColor.g, mainSpriteColor.b, Opacity * opacity);//;Mathf.Min(RadarAlpha, mainSpriteOpacity));
            ...
            if (UseArrow && ShowEdgeOverlay)
            {
                ...
                arrowImage.color = new Color(arrowColor.r, arrowColor.g, arrowColor.b, Mathf.Min(Opacity * opacity, arrowOpacity));
            }
        }
        public virtual float FadeValueLookingToMarker()
        {
            return 0;
        }
